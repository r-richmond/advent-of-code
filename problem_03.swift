...
for i in 0..<size {
	...
	for j in 0..<size {
		if arr[i][j] == 3 {
			print(i, j, abs(i - middle) + abs(j-middle), min_distance)
			min_distance = min(abs(i - middle) + abs(j-middle), min_distance)
		}
	}
}
print(min_distance)
