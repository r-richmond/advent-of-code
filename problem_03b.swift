...
for step in wire1 {
	let dir = step[step.startIndex]
	let dis_start_index = step.startIndex
	let dis = Int(step[step.index(dis_start_index, offsetBy: 1) ..< step.endIndex])!
	print(step, dir, dis, x, y)
	
	if dir == "R" {
		for i in 1...dis {
			s += 1
			arr[x + i][y] = distance_1(arr[x + i][y], s)
		}
		x += dis
	} else if dir == "L" {
		for i in 1...dis {
		...
		for i in 1...dis {
			s += 1
			arr[x][y + i] = distance_1(arr[x][y + i], s)
		}
		y += dis
	} else if dir == "D" {
		for i in 1...dis {
...
func distance_2(_ current_value: Int, _ step: Int) -> Int{
	var return_value = current_value
	if return_value < 1000000 {
		return_value += step * 1000000
	}
	return return_value
}
...
for step in wire2 {
	...
	if dir == "R" {
		for i in 1...dis {
			s += 1
			arr[x + i][y] = distance_2(arr[x + i][y], s)
		}
		x += dis
	} else if dir == "L" {
		for i in 1...dis {
		...
		for i in 1...dis {
			s += 1
			arr[x][y + i] = distance_2(arr[x][y + i], s)
		}
		y += dis
	} else if dir == "D" {
		for i in 1...dis {
...
for i in 10000..<size {
	...
	for j in 0..<size {
		if arr[i][j] > 1000000 && arr[i][j] % 1000000 != 0 {
			print(i, j, arr[i][j], calc_min(arr[i][j]), min_distance)
			min_distance = min(calc_min(arr[i][j]), min_distance)
		}
	}
}
print(min_distance)
